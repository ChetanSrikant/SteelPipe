...
export default function DashboardPage() {
  ...
  const [skuCustomerChartData, setSkuCustomerChartData] = useState({
    labels: ['Upload File & Select Sheet'],
    datasets: [
      {
        type: 'bar',
  ...
  const [skuCustomerChartOptions, setSkuCustomerChartOptions] = useState({
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
      legend: { display: true, position: 'bottom' },
      title: {
        display: true,
        text: 'SKU Count vs Total Quantity by Customer'
      },
    },
    scales: {
      y: {
  ...
  useEffect(() => {
    ...
    if (sheetSelected && hasData) {
      ...
      if (customerSales && Object.keys(customerSales).length > 0) {
          ...
          datasets: [
            {
              type: 'bar',
              label: 'Number of SKUs',
              data: sortedCustomers.map(customer => skuCounts[customer] || 0),
              backgroundColor: 'rgba(54, 162, 235, 0.6)',
              yAxisID: 'y'
            },
            {
              type: 'line',
              label: 'Total Quantity',
              data: sortedCustomers.map(customer => customerSales[customer] || 0),
              borderColor: 'rgba(255, 99, 132, 0.8)',
              backgroundColor: 'rgba(255, 99, 132, 0.2)',
              borderWidth: 2,
              tension: 0.1,
        ...
        setSkuCustomerChartOptions(prev => ({
          ...prev,
          plugins: {
            ...prev.plugins,
            title: { 
              display: true, 
              text: `SKU Count vs Total Quantity - ${availableSheets.find(s => s.originalName === selectedSheet)?.displayName || selectedSheet}` 
            }
          }
        }));
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-8">
            <div className="h-80">
              <SkuCustomerChart
                data={skuCustomerChartData}
                options={skuCustomerChartOptions}
                isLoading={isLoading}
              />
            </div>
          </div>
