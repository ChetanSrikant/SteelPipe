...
const downloadCSV = (data, filename) => {
    ...
    if (Array.isArray(data.labels)) {
        // For charts with labels (bar, line, etc.)
        ...
        // For pie charts which have different structure
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        ...
        if (sheetSelected && hasData) {
            ...
            if (pipeSales && Object.keys(pipeSales).length > 0) {
                ...
                const backgroundColors = generateColors(pipeLabels.length); // Ensure generateColors is defined
            ...
            if (customerSales && Object.keys(customerSales).length > 0) {
                ...
                // Original Customer Chart (Top Customers by Sales Value)
                ...
                setCustomerChartOptions(prev => ({
                    ...prev,
                    indexAxis: 'y', // if you want a horizontal bar chart
                ...
                // SKU Chart Data
                const skuChartLabels = customerLabels; // Reuse labels from top customers
                const skuCountValues = sortedCustomersData.map(([name]) => skuCounts[name] || 0);
                const totalQuantityOrSalesValues = customerValues; // Reuse sales values for these top customers
                    ...
                    datasets: [
                        {
                            ...
                            borderWidth: 2,
                            tension: 0.1,
                            yAxisID: 'ySku' // Ensure unique yAxisID if scales differ significantly
                        },
                        {
                            type: 'bar',
                            label: 'Total Sales Value', // Or 'Total Quantity' if customerValues represents quantity
                            data: totalQuantityOrSalesValues,
                            backgroundColor: 'rgba(255, 99, 132, 0.6)',
                            yAxisID: 'ySales' // Ensure unique yAxisID
                        }
                    ]
                });
                setSkuChartOptions(prev => ({ // Define appropriate options for combined chart
                    ...prev,
                    plugins: {
                        ...prev.plugins,
                        title: {
                ...
                // No customer data for the selected sheet
                ...
                setSkuChartData(defaultSkuChartData); // Reset SKU chart if no customer data
                setSkuChartOptions(defaultSkuChartOptions);
            }

        } else {
            // This block executes if no sheet is selected OR if essential data isn't available

            // Reset Bar Chart (Daily Pipe Sales)
            setBarChartData(defaultBarChartData);
            setBarChartOptions(defaultBarChartOptions);

            // Reset Pie Chart (Pipe Contribution)
            setPieChartData(defaultPieChartData);
            setPieChartOptions(defaultPieChartOptions);

            // Reset Customer Chart (Top Customers by Sales Value - the original one)
            setCustomerChartData(defaultCustomerChartData);
            setCustomerChartOptions(defaultCustomerChartOptions);

            // Reset SKU Chart (SKU Count vs Total Quantity/Value)
            setSkuChartData(defaultSkuChartData);
            setSkuChartOptions(defaultSkuChartOptions);

            // Reset Line Chart (Overall Monthly/Sheet Totals from file processing)
            setLineChartData(defaultLineChartData);
        }

        // Update summary state variables
    ...
    }, [selectedSheet, rawPipeSalesData, availableSheets, allMonthlyCustomerSales, /* add default state variables here if they can change and should trigger a re-render, though usually they are constant */]);
